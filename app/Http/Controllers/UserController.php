...
class UserController extends Controller
{
    ...
    public function index(){
        ...
        return view('user.userHome')->with(['pizza'=>$pizza , 'category' => $category , 'status' => $status ]);
    }

    public function pizzaDetails($id){
        $data = Pizza::where('pizza_id' , $id)->first();
        Session::put('PIZZA_INFO',$data);
        return view('user.layout.pizzadetails')->with(['pizza' => $data]);
    }
    ...
    public function categorySearch($id){
       ...
       $category = Category::get();
       return view('user.userHome')->with(['pizza' => $data , 'category'=> $category , 'status'=> $status ]);
    }
    ...
    public function order(){
        ...
        return view('user.layout.pizzaOrder')->with(['pizza'=>$pizzaInfo]);
    }
    //click order
    public function clickOrder(Request $request){
        $pizzaInfo = Session::get('PIZZA_INFO');
        $userId = auth()->user()->id;
    ...
    public function searchItem(Request $request){
        $data = Pizza::where('pizza_name' , 'like' , '%' .$request->searchData . '%')->paginate(1);
    ...
    public function searchPrice(Request $request){
        ...
        return view('user.userHome')->with(['pizza' => $query , 'category' => $category , 'status' => $status ]);
    }
    ...
    private function requestOrderData($pizzaInfo , $userId , $request ){
        ...
        return[
            ...
            'carrier_id'  => 0 ,
            'payment_status' => $request['paymentSystem'] ,
